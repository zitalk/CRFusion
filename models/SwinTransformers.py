...
class MEPISwinTransformer(nn.Module):
    def __init__(self, img_size=384, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=128, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=12, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
        ...
        self.patch_embed = LightFieldPatchEmbed(img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,norm_layer=norm_layer if self.patch_norm else None)
        print('MEPI-SwinTransformer:', patch_size)
        exit()
        num_patches = self.patch_embed.num_patches
        patches_resolution = self.patch_embed.patches_resolution
        self.patches_resolution = patches_resolution
...
class LightFieldPatchEmbed(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
...
# class LightFieldSwinTransformer(nn.Module):
#     def __init__(self, img_size=384, patch_size=4, in_chans=3, num_classes=1000,
#                  embed_dim=128, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
#                  window_size=12, mlp_ratio=4., qkv_bias=True, qk_scale=None,
#                  drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
